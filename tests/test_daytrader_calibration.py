...
def test_select_baseline_profiles_uses_deterministic_profile_rules() -> None:
    summary = pd.DataFrame(
        [
            _row(0.0, 0.75, mojofm=60.0, f1=0.20, max_ratio=0.35),
            _row(0.0, 1.0, mojofm=65.0, f1=0.24, max_ratio=0.26),
            _row(0.0, 1.25, mojofm=65.0, f1=0.22, max_ratio=0.22),
            _row(1.0, 1.25, mojofm=63.0, f1=0.23, max_ratio=0.26),
            _row(2.0, 1.25, mojofm=64.0, f1=0.22, max_ratio=0.29),
            _row(0.25, 1.0, mojofm=64.0, f1=0.22, max_ratio=0.31),
    ...
    assert profiles["selection_source"] == "daytrader_reference_calibration"
    assert profiles["selection_rule_version"] == 1
...
def _row(ssa_lambda: float, resolution: float, mojofm: float, f1: float, max_ratio: float) -> dict:
    return {
        "subject": "daytrader",

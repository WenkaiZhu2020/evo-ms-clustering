...
class LabelReassignmentMutation:
    ...
    def __init__(self, probability: float = 1.0, variable_probability: float | None = None) -> None:
                ...
                for row_index in range(len(Xp)):
                    ...
                    for var_index in np.flatnonzero(mutation_mask[row_index]):
                        ...
                        if labels[var_index] == next_label:
                            ...
                            next_label += 1
                    Xp[row_index] = _repair_labels(labels, problem.n_var)
                return Xp
    ...
    def __init__(self) -> None:
        ...
        class _Repair(Repair):
            def _do(self, problem, X, **kwargs):
                rows = [
                    _repair_labels(row, problem.n_var)
...
def repair_labels(labels: np.ndarray, class_count: int) -> np.ndarray:
    """Canonicalize labels and apply the Stage 2 admissibility repair policy."""
    return _repair_labels(labels, class_count)
...
def build_structural_problem(
    ...
    class StructuralClusteringProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs) -> None:
            labels = _repair_labels(np.asarray(x, dtype=int), len(self.class_nodes))
...
def _repair_labels(labels: np.ndarray, class_count: int) -> np.ndarray:
    labels = encoding.canonical_relabel(labels)
    if class_count <= 1:
        return labels

    labels = _ensure_min_cluster_count(labels)
    labels = _split_oversized_clusters(labels, class_count)
    labels = _merge_excess_singletons(labels, class_count)
    labels = _ensure_min_cluster_count(labels)
...
def _split_oversized_clusters(labels: np.ndarray, class_count: int) -> np.ndarray:
    labels = labels.copy()
...
def _merge_excess_singletons(labels: np.ndarray, class_count: int) -> np.ndarray:
    labels = labels.copy()
    max_singletons = max(0, int(np.floor(SINGLETON_RATIO * class_count)))
    max_size = max(1, int(np.floor(MAX_CLUSTER_RATIO * class_count)))
    for _ in range(class_count * 2):
        counts = Counter(labels.tolist())
        singleton_labels = sorted(label for label, count in counts.items() if count == 1)
        if len(singleton_labels) <= max_singletons:
            break
        singleton_label = singleton_labels[0]
        singleton_index = int(np.flatnonzero(labels == singleton_label)[0])
        target_labels = [
            label
            for label, count in sorted(counts.items(), key=lambda item: (item[1], item[0]))
            if label != singleton_label and count < max_size
        ]
        if target_labels:
            labels[singleton_index] = target_labels[0]
        elif len(singleton_labels) >= 2:
            other_label = singleton_labels[1]
            labels[singleton_index] = other_label
        else:
            break
        labels = encoding.canonical_relabel(labels)
    return labels

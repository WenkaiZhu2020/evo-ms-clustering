...
from scripts.stage3_method_body import build_semantic_graphs  # noqa: E402
...
def _load_stage3b_config() -> dict[str, Any]:
...
def _load_stage3b_graph(subject: str, class_nodes: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any], dict[str, Any]]:
    ...
    vectors, embedding_mapping, embedding_source = build_semantic_graphs.load_stage3b_inputs(subject)
...
def _identity(context: dict[str, Any], subject: str, seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "config_hash": sha256_file(STAGE3B_CONFIG),
    }
...
def run_seed(
    ...
    *,
    ...
    if destination.resolve().is_relative_to(ROOT.resolve()):
        assert_stage3b_write_path(destination, kind="optimizer result")

...
from scripts.stage3_method_body import run_seed00_optimizer as adapter  # noqa: E402
...
STAGE3_CONFIG = adapter.STAGE3B_CONFIG
...
def stage3_dir(subject: str, seed: int) -> Path:
    """Return the final Stage 3 result directory."""
    return adapter.output_dir(subject, seed=seed)


def stage3b_dir(subject: str, seed: int) -> Path:
    """Backward-compatible name for the final Stage 3 directory."""
    return stage3_dir(subject, seed)
...
def validate_final_seed(subject: str, seed: int) -> dict[str, Any]:
    ...
    if seed == 0:
        result = adapter.runtime.validate_run_output(output, context)
    else:
        from scripts.stage3_method_body.run_formal_stage3b import validate_formal_seed

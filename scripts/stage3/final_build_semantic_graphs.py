...
from scripts.stage3.final_generate_embeddings import (  # noqa: E402
...
def assert_empty_output(root: Path, *, canonical: bool) -> None:
    ...
    expected = STAGE3_GRAPH_ROOT.resolve()
...
def load_stage3_inputs(subject: str) -> tuple[np.ndarray, list[dict[str, str]], dict[str, Any]]:
    if subject not in SUBJECTS:
        raise ValueError(f"unknown subject: {subject}")
    directory = STAGE3_EMBEDDING_ROOT / subject
    paths = {
...
def create_generation_manifest(
    ...
    for subject in SUBJECTS:
        ...
        manifest["subjects"][subject] = {
            ...
            "graph_output_paths": {
                "directed": str((STAGE3_GRAPH_ROOT / subject / "directed_topk_neighbours.csv").relative_to(ROOT)),
                "edges": str((STAGE3_GRAPH_ROOT / subject / "semantic_edges.csv").relative_to(ROOT)),
                "metadata": str((STAGE3_GRAPH_ROOT / subject / "graph_metadata.json").relative_to(ROOT)),
                "mapping": str((STAGE3_GRAPH_ROOT / subject / "class_mapping.csv").relative_to(ROOT)),
                "artifact_hashes": str((STAGE3_GRAPH_ROOT / subject / "artifact_hashes.csv").relative_to(ROOT)),
            },
...
def main() -> int:
    ...
    parser.add_argument("--output-root", type=Path, default=STAGE3_GRAPH_ROOT)
    ...
    loaded = {subject: load_stage3_inputs(subject) for subject in SUBJECTS}
    results: dict[str, dict[str, Any]] = {}
    repro_results: dict[str, dict[str, Any]] = {}
    for subject in SUBJECTS:
    ...
    create_generation_manifest(
        ...
        repro_root,
        graph_config_sha256,
        "python scripts/stage3/final_build_semantic_graphs.py --repro-output-root <temporary-outside-repository>",
    )

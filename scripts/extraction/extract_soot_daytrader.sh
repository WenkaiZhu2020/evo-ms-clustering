...
if [[ -z "${SKIP_DAYTRADER_PREPARE:-}" ]]; then
  bash scripts/extraction/prepare_daytrader.sh
fi
...
EXTRACTOR_ARGS="$(PYTHONPATH=src "$UV_BIN" run --frozen python scripts/extraction/subject_extraction_config.py --subject daytrader)"

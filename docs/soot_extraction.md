...
The current implementation uses Soot class metadata and Jimple method bodies to extract class nodes, structural type dependencies, and call dependencies. It also converts selected method bodies to Shimple form and extracts scoped SSA-derived flow evidence for return-value flow and argument-passing flow. It does not run graph construction or Leiden clustering.
...
- `--classpath`: classpath for the future Soot runner.

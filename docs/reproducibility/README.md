...
- core runner and optimization source fingerprints;
...
## 9. Checksums and manifests

Formal manifests are the three `robustness_manifest.json` files inside the
formal 30-seed directories. The saved-output checksum is:

```text
results/cross_subject/03_stage2_nsga/final_statistics/formal_output_sha256sums.txt
```

Validate it from the `results/` directory so its recorded relative paths are
resolved correctly:

```bash
(cd results && sha256sum -c \
  cross_subject/03_stage2_nsga/final_statistics/formal_output_sha256sums.txt)
```

The checksum protects the saved formal snapshot. It does not authorize
rewriting formal outputs and it is not regenerated by the cleanup.
